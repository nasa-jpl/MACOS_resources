...
e2e_pie_local = fullfile(here, 'Rx', 'e2e_pie_met.in');
e2e_pie_canon = fullfile(fileparts(here), 'e2e_pie_met.in');
if ~isfile(e2e_pie_local) && isfile(e2e_pie_canon)
    copyfile(e2e_pie_canon, e2e_pie_local);
end
cd(fullfile(here, 'Rx'));
...
% --- e2e_pie: nominal + Z4 response on Elt 8 ---
fprintf('\n[bootstrap] e2e_pie nominal\n');
[p, prb, pz, pg, IZ, IG] = init_e2e_pie();
clear mex;
[~, ~, OPDnom] = call_GMI(prb, pz, pg, 0,0,0, p.pimg, IZ, IG, p);
save(fullfile(ref_dir, 'nominal_e2e_pie.mat'), 'OPDnom');
fprintf('  -> reference/nominal_e2e_pie.mat  (OPDnom RMS=%.3e)\n', ...
        sqrt(mean(OPDnom(OPDnom~=0).^2)));

% e2e_pie Z4 response: now produces a real perturbation -- the
% Zernike-apply path was silently no-op'ing because the e2e_pie Rx
...
if true
    fprintf('[bootstrap] e2e_pie Z4 response on Elt 8\n');
    p.zernSrf = [8];   % Elt 8 = a real Zernike reflector
    ...
    save(fullfile(ref_dir, 'zern_response_e2e_pie.mat'), 'dOPD');
    fprintf('  -> reference/zern_response_e2e_pie.mat  (max|dOPD|=%.3e)\n', ...
            max(abs(dOPD(:))));
end
