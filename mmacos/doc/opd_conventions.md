...
## 3. Reconciling with wavefront-sign consumers
...
## 5. Open items (flagged, not resolved here)

1. **e5pie segment pokes are inert**: `PERT` (rigid rotation or
   translation) on elements 1–7 of `e5pie` produces a bit-identical
   OPD in both the CLI and mmacos, while pokes on elements 8+ respond
   normally.  Engine-level behavior, cause not yet identified —
   needs its own investigation before segment rigid-body sensitivities
   from that deck are trusted.
