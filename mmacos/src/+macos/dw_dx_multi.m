...
%               half-width 'pf_probe_rad' centered on that field, so the
%               sphere's axis parallels the combo's own chief ray and
%               the field tilt is absorbed per block (fex physics with a
%               bundle-fit vertex; residual tilt = the coma channel).
%               Each block then harvests, and subtracts, its OWN w_nom
%               against its OWN sphere.
...
if pf_fld && isnan(pf_delta)
    ...
    if rmax == 0
        error('macos:dw_dx_multi:pfProbe', ...
