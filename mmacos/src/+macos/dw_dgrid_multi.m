...
reaim = field_stop_reaim(session, rx_path, opts.stop_elt, []);
...
if pf_fld && isnan(pf_delta)
    ...
    if rmax == 0
        error('macos:dw_dgrid_multi:pfProbe', ...
...
per_field_dwdg   = cell(n_cfg, n_fields);
...
for k = 1:n_fields
    ...
    elseif ~isequal(names, sf.channel_names)
        error('macos:dw_dgrid_multi:channelMismatch', ...
...
if has_cfg
    ...
    drift = config_axis('assert', session, snap, cfgs(ic).name, 'dw_dgrid_multi');
...
for k = 1:n_fields
    ...
    dwdgall(global_rows, :) = per_field_dwdg{ic, k}(loc, :);
...
if ~isempty(ctr_idx)
  ...
  for ic = 1:n_cfg
    ...
    dwdgall_ctr = dwdgall(row0 + find(in_ctr), :);
    dwdg_C = per_field_dwdg{ic, ctr_idx};
...
if has_cfg
    ...
    out.per_field_dwdg       = per_field_dwdg;      % Nc x Nf
    ...
    out.per_field_dwdg       = per_field_dwdg(1, :).';
