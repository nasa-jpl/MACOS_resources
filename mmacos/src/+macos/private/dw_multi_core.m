...
for k = 1:n_fields
    ...
    if nnz(sf.w_nom_2d) == 0
        ...
        if n_empty == 1
            ...
            % 2026-09-09: the raw engine read at the pupil had 12756
            % rays and max|w| 1e-4 while this canvas was all zeros --
            % the warning below used to assert "0 rays survived", which
            % was false there).  Ask the engine how many rays are alive
            % at the read surface right now and say which case this is.
            try
                st = session.trace(sf.wf_elt);
                rs = macos.get_ray_status(st.nRays);
                n_pass = nnz(rs.status == 0);
                Wraw = macos.opd(sf.wf_elt);
                raw_nnz = nnz(Wraw);  raw_max = max(abs(Wraw(:)));
            catch
                n_pass = NaN;  raw_nnz = NaN;  raw_max = NaN;
            end
            if n_pass > 0 && raw_nnz > 0
                why = sprintf(['the ENGINE read is NOT empty (%d rays ' ...
                    'pass, %d non-zero samples, max|w| %.3g): this is a ' ...
                    'canvas/mask fault in the supervisor, not vignetting ' ...
                    '-- report it'], n_pass, raw_nnz, raw_max);
            else
                why = sprintf(['%d rays pass at the read surface: full ' ...
                    'vignetting or a tight clip aperture at this field'], ...
                    n_pass);
            end
            warning(eid_(F, 'emptyOPD'), ...
                ['%sfield %s: OPD canvas at the read surface (elt %d) ' ...
                 'has no non-zero samples; this block contributes 0 ' ...
                 'rows to the stacked Jacobian.  %s.  Further empty ' ...
                 'blocks in this run are counted, not re-warned.'], ...
                cfg_tag(has_cfg, cfgs, ic, F), fields(k).name, sf.wf_elt, why);
        end
    end
