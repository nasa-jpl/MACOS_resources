...
~ THE METRIC CONTRACT, once for the whole deck: strict RMS WFE, centroid reference on the stage's frozen focal plane, exit-pupil anchor, piston-only removal; quoted statistic = dense 11x11 map MAXIMUM over the field box.  Every WFE number on every slide inherits it.  Workbook genre: 18 content slides in six sections (budget deviation declared, DECK2_PLAN.md).  DRAFT — pending sign-off.
...
![The honest re-solve (probe C): 45.4 nm max, true floor 33.4 mm.](s5_budget/s5b_legC_map.png){h=2.35}
...
## F16 — The sequel: fix the frictions, then walk the box | The cold start dies 205 mm inside the glass; the walk lands 69.8 nm — 8531x better
::: left
...
| field box | 20x20° at YAN +22° | 15x15° at YAN +22.5° |
| clearances | ≥ 50 / 35 mm | ≥ 40 / 25 mm |
| envelope | native .seq spacings | rodgers3 W-fold x1.65 |
| exit pin | horizontal | horizontal |
- Act 2 — the REDEMPTION rerun (run_t5r): ZERO crashes, every failure self-diagnosed (capped S1 74.9 nm; "INVALID — 104/121 fields lost"; exit "unmeasurable") — but S3–S5 stall at ~596 µm with the beam 205 mm INSIDE the glass (figure): the cold start is outside the convergent basin, and both counter-designs stall with it.
![The stall, drawn: the corner-field fans thread the mirrors — the layout no number can excuse.](../../templates/10_telescopes/offset_imager/t5_redemption/t5r_s4_layout.png){h=1.8}
::: right
- Act 3 — the WALK (oi_walk): solve an easy 5° box, then open the box toward the target carrying each solved design as the next warm start — full freedom + all constraint rows at every step; every carried design traced the widened box first try.
...
![The two trends the table hides: WFE climbs as the box opens, the floor falls through the 25 mm gate — and the endgame re-solve (stars, next slide) closes both.](walk_trend.png){h=2.45}
...
## F17 — The endgame: the last gate closes at negative cost | Hull reads the walk endpoint at 28.7 mm (clears as-is); the re-solve lands 47.1 nm at 30.9 mm — better than the walk on BOTH axes
::: left
- WHY the walk left 7.2 mm on the table: oi_solve's plateau break watches WFE only, so the final step stopped at 6 iterations — the moment WFE flattened, with the clearance hinge still pulling.  Under-solved, not blocked.
- The re-solve: warm from the 13° step, a restart loop past the break, and the clearance hinge asked for MORE than the gate — at-spec undershoots, because the hinge gradient dies at its own target:

| hinge (mm) | map max (nm) | floor (mm) | iters / restarts | ≥ 25 mm |
| 25 (spec) | 51.7 | 24.2 | 46 / 3 | short |
| 32 | 47.1 | 30.9 | 82 / 5 | yes |
| 40 | 47.3 | 39.0 | 92 / 6 | yes |
- Envelope stretch is the WRONG lever: x1.75–x1.9 buys WFE (34.9 -> 31.3 nm) while the at-spec floor asymptotes just under 25 mm — and the x1.65 envelope already reaches 30.9.  Keep the envelope; raise the hinge.
::: right
![The closing map, ladder-style: 47.1 nm max over the 15° box at a 30.9 mm floor (deck t5_endgame_wfe.in, committed).](../../templates/10_telescopes/offset_imager/t5_endgame/t5_endgame_wfe_map.png){h=3.35}
- Every frontier box is now spec-compliant: the deficit was part model conservatism (hull 28.7 mm), part under-solve — and zero physics.
