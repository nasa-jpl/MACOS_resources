...
|---|---|---|---|
| A1 | `challenges/rodgers1` stages 3 & 4 (`run_epd4060.m`) | the case that found this: +0.5° offset, FPA tilted 14.3°; merit artifact ~22× the WFE | **improves.** His own conics score 115.3 nm max where ours score 181.2 (Addendum 4 §B); his rigid body is 1.83× deeper than ours. Target: S3 ≤ ~115 nm, S4 ≤ ~1.5× his 39.8 |
...
| A7 | `templates/10_telescopes/tma_centered/{tma_centered_foldfp,tma_centered_fold_search}.m` | fold + FP extraction ⇒ tilted detector | improves |
| A8 | `templates/10_telescopes/wf2_freeform/wf2_freeform.m` | wide-field freeform at a field bias | improves |
| A9 | `templates/10_telescopes/tma_freeform/tma_freeform.m`, `freeform_unobscured` | freeform solves over a biased box | improves; freeform DOFs have more freedom to chase the artifact, so possibly the largest change on the list |
| A10 | `templates/10_telescopes/sz_tma` (sphere+Zernike TMA) | biased box, Zernike DOFs; same reasoning as A9 | improves |
| A11 | `templates/10_telescopes/tma_3plus1/{tma_3plus1,tma_3plus1_optimize,tma_3plus1_aoi_search}.m` | 3+1 with a fold; detector not normal | improves |
...
| A14 | `templates/10_telescopes/tma_widefield/example_tma_widefield.m`, `rc_offaxis/example_rc_offaxis.m` | off-axis / wide-field | improves |
