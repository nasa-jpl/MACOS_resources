...
%   Runtime: ~4 min (S1-S3, 2 GN iterations each, 3x3 maps, nGridpts 21).
%
%   See also OFFSET_IMAGER, OFFSET_IMAGER_PARAMS, tests/tRodgers3.m.
    ...
    methods (TestClassSetup)
        function setup(tc)
            ...
            tc.OUT = offset_imager(struct( ...
                ...
                'seed_R1_m',15, ...
                'stages',1:3, 'gn_iters',2, 'map_n',3, 'nsolve',2, ...
    ...
    methods (Test)
        ...
        function test_s2_offset_is_a_disaster(tc)
            tc.verifyGreaterThan(tc.OUT.s2.cost_of_offset, 3.0, sprintf( ...
        ...
        function test_s3_resolve_recovers(tc)
            tc.verifyLessThan(tc.OUT.s3.map.max_nm, 0.5*tc.OUT.s2.map.max_nm, ...
                'the S3 re-solve at the used field failed to recover');
        end
    end
end
