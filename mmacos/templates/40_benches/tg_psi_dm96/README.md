...
## S6 COLOR (tg96_s6color.m, 2026-09-08): the IFO has NO chromatic lever

Dave's ask ("try running both systems at multiple colors, maybe the
combination will help with some poor SNR regions"), IFO twin of
`../zwfs_dm96/zwfs_s6color.m`.  Five colors (480/532/632.8/700/780 nm;
only the deck header `Wavelen=` changes; the QWPs are re-set to lam/4 at
each color by `macos.waveplate`, i.e. ideal ACHROMATIC plates, and the
lenses are dispersionless), each with its own analyzer bases, null
reference, anchor and 12-mode transfer through the true-kernel
estimator; 96x96, model 1024 / NGRID 385; combination by the
multi-channel Wiener (`../dm_gauge_lib/dmg_color_comb`, 'radial').

**Result: nothing moves.**  The modal transfer is identical across the
five colors to three decimals (1.015 at 0.7 cyc/ap -> 0.576 at 56.6 at
EVERY wavelength; the (48,0) row 0.8996/0.8964/0.8999/0.8966/0.8997),
and every row is identical: flat hold-out 92 pm at each color and 92
for the five-color combination; dense random 10 nm 4171 -> 4174;
single 10 nm on the 30 nm base 46 -> 46 pm (SNR 210 -> 210); grid 47 x
1 nm on base SNR 19.8 -> 19.8.  The 632.8 column reproduces S3
exactly (0.9171 / 92 pm) and the K=1 combiner reproduces
`dmg_modal_corr` to 4.6e-13 pm.

**Why, and what it tells us.**  A diffraction MTF scales with lambda --
between 480 and 780 nm the pupil-image resolution would move by 1.6x
and the 0.58 roll-off at 57 cyc/ap with it; it moved by < 0.3%.  So the
IFO's high-frequency deficit (the dense-random gain 0.77-0.83, the
weak axis since run 10) is GEOMETRIC: the null-tuned tail's imaging
conjugate (checker gain 0.835 -> 0.504) and its 0.136 mm mapping
distortion blur the pupil image ray-by-ray, which no wavelength
changes.  In the model as built this is also by construction -- the
TG96 deck is `PropType= Geometric` at every element, so the detector
field is the ray-mapped wavefront and the four-step recovers
phi*lam/(4 pi) exactly at any lambda -- but the geometric treatment is
the right one here: the field lens (20.6 mm aperture at the internal
focus) sets a diffraction resolution of ~13 um at the DM against 1 mm
actuators, so a physical-optics tail would add nothing visible.  The
colors therefore confirm the run-10 diagnosis: the IFO's next lever is
the JOINT tail objective (null + transfer + distortion), not the
source.  Contrast the ZWFS, whose transfer nulls are set by the
dimple's angular size and migrate as 1/lambda -- there the five-color
combination fills them (zwfs_dm96 README, S6).

Housekeeping: the `null_pm` column printed 0.0 in this run by
construction (it measured meas(flat), which is referenced to the null
phase); the script now reports the raw null-phase map instead.
Per-color decks `tg96_{test,ref}_<nm>nm.in` are regenerated by the
script (gitignored).
