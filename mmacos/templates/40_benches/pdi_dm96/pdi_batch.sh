...
if [ -z "${ZWFS_NOWAIT:-}" ]; then
    ...
    if command -v flock >/dev/null 2>&1; then lockcmd="flock $lockdir/.batch.lock"; else lockcmd=""; fi   # macOS has no flock (2026-09-16: the Mac run died with exit 127); the wait loop above still serializes
else
    lockcmd=""
fi
