function OUT = cf3f_feather(over)
%CF3F_FEATHER  The FEATHERED d=1.10 restart ladder (Dave 2026-09-01):
%   a few-pixel Gaussian roll-off on every hard edge of the pupil --
%   segment edges, gaps AND the circular stop -- in place of the
%   throughput-hungry prolate.  cf_chain apod_kind 'feather'
%   (blur sigma feather_px, remapped so transmission reaches ZERO at
%   each binary edge; see cf_chain's feather_).
%
%   Everything else is the CF3d configuration held: hard occulter,
%   Lyot 0.90, circular stop c098 in every pass including the bare
%   normalizer.  Ladder: widened alphas from the start, target 5e-11,
%   two-stall convergence, wall-clock bounded.  Writes cf3f_mask.png
%   (the feather mask + its slice) at build.
%
%   over.cf3f fields as cf3e's; plus feather_px (2).
%
%   See also CF3E_NOAPOD, CF3D_DEEPDIG, CF_CHAIN.
    ...
    ov = over;  c3 = struct();
    if isfield(ov,'cf3f'), c3 = ov.cf3f;  ov = rmfield(ov,'cf3f'); end
    ...
    rep = fullfile(P.outdir, 'cf3f_report.txt');
    logf_(rep, '==== e2e6m CF3f -- FEATHERED d=1.10 restart ladder (sigma %.1f px) %s', ...
    ...
    assert(~isempty(ia), 'cf3f: apod_kind not in the apl config');
    cfg{ia+1} = 'feather';
    cfg = [cfg, {'feather_px', c3.feather_px}];
    prop_in = fullfile(P.outdir, 'r1_seg_d110_prop.in');
    dmrx    = fullfile(P.outdir, 'r1_seg_d110_dm.in');
    assert(isfile(prop_in) && isfile(dmrx), 'cf3f: run cf3b first (d110 decks)');
    ...
    assert(~isempty(ch.masks.S), 'cf3f: circular stop MISSING at the apodizer plane');
    A = ch.masks.A;
    assert(~isempty(A) && min(A(:)) >= 0 && max(A(:)) <= 1, 'cf3f: feather mask malformed');
    frac_soft = nnz(A > 0.02 & A < 0.98) / max(nnz(A > 0.02), 1);
    assert(frac_soft > 0.05, 'cf3f: mask has no soft edge band -- feather did not apply');
    ...
    exportgraphics(fm, fullfile(P.outdir,'cf3f_mask.png'), 'Resolution', 130);
    close(fm);
    ...
    % ---- resume or start ------------------------------------------------
    ckpt = fullfile(P.outdir, 'cf3f_run.mat');
    ...
    for r = r0:c3.max_rounds
        tr = tic;
        Gc = fullfile(P.outdir, sprintf('cf3f_G_%s_r%d.mat', tag, r));
    ...
    logf_(rep, 'CF3f DONE in %.1f h: %.3e -> %.3e over %d rounds (lin-ach %.3e; thru %.3f vs apl 0.091 / bare 0.745)', ...
