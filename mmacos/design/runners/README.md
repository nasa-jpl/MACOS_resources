...
| `run_sensitivities` | **SHIPPED** | (segmented) `.in` | `*_sens.mat` (dwdx/dwdz/dwdgrid, + dwdsurf opt-in) + conditioning report + per-element pages in `*_pages/`, sized so every map is readable at any segment count (`panel_in`/`tile_in`; paginated on element boundaries) and listed in `*_pages_index.txt`.  `groups` / `groups_auto` (+ the four `group_*` knobs) append RIGID-BODY element-group columns to the **dwdx** rung only — one 6-DOF block per group, after the per-element block; group channels carry `iElt` 0 and `kind` `'Group'`, and get their own page |
