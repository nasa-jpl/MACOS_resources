...
% s4_jacobians.mat is >20 MB and gitignored -- write the committed
% fingerprint sidecar (jac_fingerprint) so the generation is auditable
% without the blob (generation stamps + per-column norms + headline dims).
jac_fingerprint('write', fullfile(here, 's4_jacobians.fp.json'), ...
    struct('dwdxall', ox.dwdxall, 'dwdzall', oz.dwdxall, ...
           'dwdgall', og.dwdxall), ...
    struct('product', 's4_jacobians', 'reset_xp', ox.reset_xp, ...
           'rx', sprintf('e2e_%s.in', v), 'fov_arcmin', P.inst.fov_arcmin));
