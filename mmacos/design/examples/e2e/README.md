...
| `s7_simulator.m` | s6 | PLANNED `run_simulator`: PSF simulator — mmacos engine OR linear model (user switch) driven by an x/z/grid time history, with a **SINGLE-STEP STATIC estimator** (Dave 2026-07-19; the OSE form `x̂ = x̄ + K·(m − m̄)`, converged steady-state gains — the `dxde`/`dxdl` MMSE partitions s5 already exports — with `m = [l; e]`) and the OSC regularized wavefront-nulling controller on `dwdu`, so UNCONTROLLED and CONTROLLED outputs both emerge.  Background: `MACOS_sandbox/Documents/OSE_Eqns_2019.pdf` + `2025_JATIS_HWO_Special_Issue-2.pdf` |
