...
design/
  ...
  runners/    the STAGE RUNNERS (design -> segmentation -> sensitivities
              -> MET -> compare -> simulate)
```

The adaptable EXAMPLES themselves moved out of this tree in the 2026-08
reorganization: they are **templates** now, under
[`../templates/`](../templates) (the telescope progression below is
[`../templates/10_telescopes/`](../templates/10_telescopes)), and the
worked design challenges are under
[`../challenges/`](../challenges).  This directory holds only library
code -- a folder of examples must never house the helpers it calls.
...
are distinct from the fixed feature demos elsewhere under
[`../templates/`](../templates):
...
|---|---|---|
| **1. Two-mirror** | [`rc_onaxis/`](../templates/10_telescopes/rc_onaxis) (obscured aplanat), [`rc_unobscured/`](../templates/10_telescopes/rc_unobscured) (eccentric-pupil section), [`wf2_freeform/`](../templates/10_telescopes/wf2_freeform) (freeform on M2) | Conic seeding, the RC astigmatism field limit, the off-axis-section recipe, what one freeform surface buys |
| **2. Three-mirror** | [`tma_centered/`](../templates/10_telescopes/tma_centered) (**obscured family** + the obscured-vs-unobscured A/B), [`tma_onaxis/`](../templates/10_telescopes/tma_onaxis) (obscured Korsch, bias sweep), [`tma_offaxis/`](../templates/10_telescopes/tma_offaxis) (**unobscured family**, conic), [`tma_freeform/`](../templates/10_telescopes/tma_freeform) (freeform M2+M3), [`sz_tma/`](../templates/10_telescopes/sz_tma) (all-sphere + Zernike, real intermediate focus) | The third mirror nulls field astigmatism; the two BASIC families — centered/obscured (symmetry intact, wide field, JWST-like) vs unobscured section (clear pupil bought with induced binodal field astigmatism); intermediate focus; convex-secondary convention; sphere+Zernike decoupling |
| **3. Three + one** | [`tma_3plus1/`](../templates/10_telescopes/tma_3plus1) *(reference design, in progress)* | An M4 **field mirror** just past the science focus relays the exit pupil to an accessible plane and **flattens** it — the coronagraph-ready front end (flat pupil conjugate for DM/apodizer/Lyot, intermediate focus for an FPM) |
...
run('~/dev/MACOS_resources/mmacos/templates/10_telescopes/tma_offaxis/tma_offaxis.m');
```
